...
public class Helper {
    ...
    public static void printCloudletList(List<Cloudlet> list) {
        ...
        Log.println();
        Log.println("========== OUTPUT ==========");
        Log.println(String.format("%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s",
                "Cloudlet ID", "STATUS", "Data center ID", "Host ID", "VM ID", "Time", "Start Time", "Finish Time"));

        DecimalFormat dft = new DecimalFormat("###.##");
        for (Cloudlet value : list) {
            cloudlet = value;
            if (cloudlet.getStatus() == Cloudlet.CloudletStatus.SUCCESS) {
                Log.println(String.format("%-15d%-15s%-15d%-15d%-15d%-15s%-15s%-15s",
